...
갱신은 자동입니다. GitHub Actions가 매주 인증 데이터를 다시 수집하고 새 인증이 나타나면 읽기 전용 AI 조사로 고신뢰 매핑만 보완한 뒤 PR을 만듭니다. PR의 실제 HEAD는 별도의 읽기 전용 AI 검증을 거쳐 `이대로 머지 가능` 또는 `추가 조치 필요` 코멘트를 받습니다. AI 조사나 검증이 실패해도 원본 동기화 PR은 유지되며 사람이 최종 확인해 머지합니다.

- AI 모델은 `gpt-6-astra`로 고정하며 저장소 변수 `CODEX_SYNC_MODEL`로 변경할 수 있습니다. 기존 `OPENAI_API_KEY`가 속한 OpenAI 프로젝트의 허용 모델 목록에도 해당 모델이 있어야 합니다. 403 접근 권한 오류는 API 키 교체나 재시도만으로 해결되지 않습니다.
- 웹 근거는 신뢰된 워크플로의 별도 Responses API `web_search` 호출로 수집합니다. 사전 보완과 PR 사후 검증은 각각 독립 조사하며, 읽기 전용 Codex에는 네트워크 대신 출처·검증 대상 SHA가 포함된 `sync-evidence.json`을 제공합니다. PR 코드에는 원본 API 키를 전달하지 않습니다.
- 웹 수집 상한은 단계당 대상 20개/초기 입력 60,000자, API 요청 1회, 웹 도구 호출 12회, 출력(추론 포함) 8,000토큰, 8분입니다. 검색 결과의 입력 토큰은 별도 발생하며 사용량을 실행 요약과 artifact에 기록합니다. 대상이 없으면 API를 호출하지 않습니다. 상한 초과·검색 실패·누락 시 자동 확정을 건너뛰며 수동 검토가 필요합니다. 근거 artifact는 7일 보관합니다. 이후 Codex 검증의 사용량은 이 웹 수집 상한과 별개입니다.
- 같은 KST 날짜에 원본 수집·검증·PR 게시가 성공했다면 이후 예약 슬롯은 생략합니다. AI 검토·코멘트만 실패했을 때는 원본을 다시 수집하지 않습니다.
- 매핑 수동 보완이나 권한 수정 후에는 Actions의 `sync-kencis`에서 `review-only`를 켜서 실행하세요. 열린 `sync/kencis → main` PR의 최신 HEAD만 검증하므로 기존 보완 커밋을 덮어쓰지 않습니다. 코멘트의 검증 대상 SHA가 현재 HEAD인지 확인한 뒤 머지하세요.
